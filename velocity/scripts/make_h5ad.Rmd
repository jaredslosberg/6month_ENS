...
```{r}
cds <- readRDS(here("6mo_MENS.rds"))

plot_cells(cds, color_cells_by = "tricyclePosition") #not expecting much for velocity based on no cycling cells
...
exprs_mat_list <- pmap(list(spliced_status, spliced_status_full), function(spl, spl_full){
  ...
  exprs_mat_sep <- map(samples, function(samp){
    ...
    genes_trimmed <- genes %>% str_split_fixed(., "\\.", 2) %>% .[,1]
  ...
  exprs_mat_filt <- exprs_mat[cds_genes,]
...
rownames(cds) <- fData(cds)$gene_short_name
writeH5AD(cds, file = here("6mo_MENS.h5ad"), X_name = "counts")
```
