...
transferred_cell_weights_filt <- transferred_cell_weights %>%
  select(all_of(paste0("cellPattern", 32)))


cell_sample <- sample(rownames(transferred_cell_weights_filt), 5000)
labels <- pData(cds)[cell_sample, "Age_binned"] %>%
  as.character() %>%
  setNames(cell_sample)

emd_res <- calculate_emd(transferred_cell_weights_filt[cell_sample, , drop =F], labels, binSize = 0.0001)
