...
pData(cds)[,"cell_type_aggregate"] <- pData(cds)[,"cell_type"] %>%
  str_replace_all(., "neuroglia/neuroendocrine", "NENs")

cell_types_to_keep <- c("MENs","NENs","Neuroglia")

#if needed, rename cell types for figure
cds_sub <- cds[,pData(cds)$cell_type_aggregate %in% cell_types_to_keep]
...
pData(cds_sub) <- pData(cds_sub) %>% as.data.frame() %>%
  group_by(cell_type_aggregate) %>%
  mutate(group_title = paste0(cell_type_aggregate, " (n = ", n() ,")")) %>%
  ungroup() %>%
  DataFrame()
  
#
genes <- c("Calcb","Aebp1","Cdh3","Cftr","Clic3","Fmo2","Ntf3","Slpi","Smo","Myl7","Met","Il18","Slc17a9","Ret","Nos1","Sox10", 
           "Stmn2","Elavl1","Elavl2","Elavl3","Elavl4","Ncam1","Stx3","Vsnl1","Hand2", "Pde10a", "Malat1")
...
pdf(here("plots/6mo_manuscript/quasiviolin_expression_p20.pdf"), width = 9, height = 24) 
dots
dev.off()
...
pdf(here("plots/6mo_manuscript/quasiviolin_expression_p20_modcolors.pdf"), width = 9, height = 24) 
dots
dev.off()
...
pdf(here("plots/6mo_manuscript/quasiviolin_expression_p20_allgenes.pdf"), width = 9, height = 8) 
dots
dev.off()
...
pdf(here("plots/6mo_manuscript/sparklines_expression_allgenes.pdf"), width = 9, height = 8) 
sparks
dev.off()