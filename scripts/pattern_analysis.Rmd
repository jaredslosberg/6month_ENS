...
#TODO: SOMETHING REALLY WONKY HAPPENING HERE
geneWeights.df.filt <- geneWeights.df[which(rownames(geneWeights.df) %in% fData(lmmp_6mo)$gene_short_name),]

geneWeights.df.filt <- geneWeights.df.filt[fData(lmmp_6mo)$gene_short_name,]

assertthat::assert_that(all(rownames(geneWeights.df.filt) == fData(lmmp_6mo)$gene_short_name))
geneSetEnrichment(gene_weights = geneWeights.df,
                  n_genes = 1000, n_patterns = 50, file_prefix = paste0("/home/jared/ENS/6mo_LMMP/results/GSEA/lmmp/lmmp_6mo_",date,"_"))
```
