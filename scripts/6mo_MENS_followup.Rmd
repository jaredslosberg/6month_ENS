...
source("./accessory_functions/cluster_genes_mod.R")
source("./accessory_functions/transferAnnotations.R")
...
MENS_cell_names <- read.csv("../results/MENS/6mo_mens_names.txt", header = T)
MENS_cell_names <-as.character(MENS_cell_names$x)

MENS <- lmmp[,MENS_cell_names]
...
MENS <- align_cds(MENS, num_dim = 10, alignment_group = "batch",alignment_k = 20,verbose=T)
MENS <- reduce_dimension(MENS,max_components=2,cores=4,verbose=TRUE)
...
htmlwidgets::saveWidget(dt, file = "../results/MENS_subset_cluster_markers.html")
...
cc_features <- c("cycle_phase","cycle_G1_score","cycle_S_score","cycle_G2M_score")

for(feat in cc_features){
  MENS <- transferAnnotations(to_annotate_cds = MENS, annotated_cds = lmmp, by_cluster = F, annotation_name = "feat")
...
write.csv(lrt, file = "../results/MENS/de_genes_by_cluster.csv")
...
pdf(paste0("../plots/NMF/MENS/MENS_",date,"_k_40_log_cellsize_cellweights.pdf"), width = 10)
...
write.csv(cellWeights.df, paste0("../results/NMF/MENS/MENS_",date,"_log_cellsize_pattern_cell_weights.csv"))

write.csv(geneWeights.df, paste0("../results/NMF/MENS/MENS_",date,"_log_cellsize_pattern_gene_weights.csv"))
...
pdf(paste0("../plots/NMF/MENS/MENS_",date,"_pattern_11-25_categorical_feature_correlation.pdf"), height= 8, width = 8)
...
geneSetEnrichment(gene_weights = geneWeights.df, n_genes = 1500, n_patterns = 40,
                  file_prefix = paste0("../results/GSEA/mens/MENS_",date,"_pattern_gene_weights"))

```

```{r}
saveRDS(MENS, file = "../6mo_MENS_11-25.rds")
sessionInfo()
```
