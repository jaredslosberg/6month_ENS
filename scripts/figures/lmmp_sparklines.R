### Visualize expression of IHC-matched and neuronal genes via sparklines in MENs/NENs/glia
### Visualize expression of neuronal marker genes in MENs/NENs/glia
...
colors <- c("#E68613","#ABA300","#C77CFF") %>% set_names(labels)
...
# colors_replace <- heatmap_colors %>% setNames(colors)
# 
# 
# annotDF_mod[,"primary_type_color"] <- annotDF_mod[,"primary_type_color"] %>% str_replace_all(.,colors_replace)

genes2 <- c("Calcb","Cdh3","Aebp1","Cftr","Clic3","Fmo2","Ntf3","Slpi","Smo","Il18","Upk3b","Pde10a","Elavl2","Elavl3","Elavl4","Stx3", 
...
pdf(here("plots/supp_figures/quasiviolin_expression.pdf"), width = 11, height = 15) 
dots
dev.off()
...
genes_all <- c(genes, genes_add) %>% unique
genes_all_list <- shorten_gene_lists(genes_all, 8)



annotDF <- prep_annotation_matrix(cds_sub[fData(cds)$gene_short_name %in% genes_all,], 
                                          facet_by = "cell_type", colors = colors, labels = labels)
matDF <- prep_expression_matrix(cds_sub[fData(cds)$gene_short_name %in% genes_all,],
                                norm_method = "none",exprs_assay = "logcounts" ,
                                exprs_mat_gene_names = "gene_id", short_gene_names = "gene_short_name")


dots <- scrattch.vis::group_quasirandom_plot(matDF, annotDF, genes = genes_all,
                                     grouping = "primary_type",
                                     label_height = 2, 
                                     font_size = 10, 
                                     max_width = 15)
...
pdf(here("plots/supp_figures/quasiviolin_expression_all.pdf"), width = 9, height = 124) 
dots
dev.off()
...
sparks <- scrattch.vis::sample_bar_plot(matDF, annotDF, genes = genes_all,
                              grouping = "primary_type",
                              label_height = 2, 
                              font_size = 10, 
                              max_width = 15,
...
pdf(here("plots/supp_figures/sparklines_expression_all.pdf"), width = 9, height = 124) 
sparks
dev.off()
...
annotDF <- prep_annotation_matrix(cds_sub[fData(cds)$gene_short_name %in% genes_2,], 
                                  facet_by = "cell_type", colors = colors, labels = labels)
matDF <- prep_expression_matrix(cds_sub[fData(cds)$gene_short_name %in% genes_2,],
                                norm_method = "none",exprs_assay = "logcounts" ,
                                exprs_mat_gene_names = "gene_id", short_gene_names = "gene_short_name")

pdf(here("plots/supp_figures/selected_genes_sparklines_snap25.pdf"), width= 10, height = 4)
scrattch.vis::sample_bar_plot(matDF, annotDF, genes = genes_2,
                              grouping = "primary_type",
                                     label_height = 30, 
                                     font_size = 10, 
                                     max_width = 15, bg_color = "#FFFFFF")
dev.off()
