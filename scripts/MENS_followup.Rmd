...
source("./scripts/accessory_functions/monocle_mods.R")
source("./scripts/accessory_functions/doDifferentialExpression.R")
source("./scripts/accessory_functions/pattern_plotting.R")
source("./scripts/accessory_functions/geneSetEnrichment.R")
source("./scripts/accessory_functions/cluster_genes_mod.R")
...
```{r}
#MENS subset
cds_raw <- readRDS("./TC_LMMP_prefilt.rds")

#After running the above script and determining which quality cells are in the MEN population, subset on these
MENS_cell_names <- read.csv("./results/MENS/mens_names.txt", header = F)
MENS_cell_names <-as.character(MENS_cell_names$V1)

MENS <- cds_raw[,MENS_cell_names]

MENS <- estimate_size_factors(MENS)
MENS <- preprocess_cds(MENS, num_dim = 5, verbose=TRUE)
...
pData(MENS)$age<-"p10"
pData(MENS)$age[grepl("^p2",pData(MENS)$sample)]<-"p20"
pData(MENS)$age[grepl("^p6",pData(MENS)$sample)]<-"P60"

pData(MENS)$batch<-"0"
pData(MENS)$batch[pData(MENS)$sample %in% c("p11","p21","p61")]<-"1"
pData(MENS)$log10UMI <- log10(pData(MENS)$Total_UMIs)


MENS <- align_cds(MENS, num_dim = 5, alignment_group = "batch",alignment_k = 20,verbose=T)
MENS <- reduce_dimension(MENS,max_components=2,cores=4,verbose=TRUE)
MENS <- cluster_cells(MENS, k=20, cluster_method="leiden",num_iter=5, reduction_method="UMAP", partition_qval=0.05,verbose=TRUE,resolution=1e-04)
plot_cells(MENS)
plot_cells(MENS, color_cells_by = "sample", label_cell_groups = F)
...
pData(MENS)$assigned_cluster[pData(MENS)$assigned_cluster == 6] <- 3
...
```{r markers, eval = F}
marker_test_res <- top_markers(MENS, group_cells_by="assigned_cluster", cores=8,verbose=T)
...
htmlwidgets::saveWidget(dt, file = "./results/MENS_subset_cluster_markers_11_5.html")
...
exprs.mat <- exprs(MENS)
rownames(exprs.mat) <- fData(MENS)$gene_id_trimmed
cyc_results <- cyclone(exprs.mat, pairs = mm.pairs, verbose = T)

pData(MENS)$cycle_phase <- cyc_results[['phases']]

pData(MENS)$cycle_G1_score <- cyc_results[['normalized.scores']]$G1
pData(MENS)$cycle_S_score <- cyc_results[['normalized.scores']]$S
pData(MENS)$cycle_G2M_score <- cyc_results[['normalized.scores']]$G2M
...
plot_cells_mod(MENS, color_cells_by="age", group_cells_by="cluster", label_cell_groups = F) +
  ggtitle("Age") + facet_wrap(vars(age))
plot_cells_mod(MENS, color_cells_by="batch", group_cells_by="cluster", label_cell_groups = F) +
...
p <- plot_cells(cds, color_cells_by = "age", label_cell_groups = F)
g <- plot_cells(cds[, pData(cds)$cell_type == "MENS"],cell_size = 1.5, color_cells_by = "age", label_cell_groups = F)
p1 <- plot_cells(cds[,pData(cds)$sample=="p10" & pData(cds)$cell_type == "MENS"],cell_size = 1.5, color_cells_by = "age", label_cell_groups = F) + scale_color_manual(values = my_colors[1])
p2 <- plot_cells(cds[,pData(cds)$sample=="p11" & pData(cds)$cell_type == "MENS"],cell_size = 1.5, color_cells_by = "age", label_cell_groups = ) + scale_color_manual(values = my_colors[2])
p3 <- plot_cells(cds[,pData(cds)$sample=="p20" & pData(cds)$cell_type == "MENS"],cell_size = 1.5, color_cells_by = "age", label_cell_groups = ) + scale_color_manual(values = my_colors[3])
p4 <- plot_cells(cds[,pData(cds)$sample=="p21" & pData(cds)$cell_type == "MENS"],cell_size = 1.5, color_cells_by = "age", label_cell_groups = ) + scale_color_manual(values = my_colors[4])
p5 <- plot_cells(cds[,pData(cds)$sample=="p60" & pData(cds)$cell_type == "MENS"],cell_size = 1.5, color_cells_by = "age", label_cell_groups = ) + scale_color_manual(values = my_colors[5])
p6 <- plot_cells(cds[,pData(cds)$sample=="p61" & pData(cds)$cell_type == "MENS"],cell_size = 1.5, color_cells_by = "age", label_cell_groups = ) + scale_color_manual(values = my_colors[6])
...
write.csv(lrt, file = "./results/MENS/de_genes_by_cluster_11_5.csv")
...
```{r defineSubclusters}
#For genes that are differentially expressed, what are there specificities to each cluster
MENS_sub <- MENS[de_genes,]
marker_test_res <- as.data.frame(top_markers(MENS_sub, group_cells_by="clusters", cores=8,verbose=T))

DT::datatable(marker_test_res %>% dplyr::select(-c(gene_id, marker_score,pseudo_R2)), options = list(pageLength = 15))

#Cluster 6 defined by proliferative markers
#Clusters 3 and 6 have an order of magnitude more UMI per cell


##what is actually driving these clusters to be different? Because they seem very similar.
##Try removing mt and ribosomal proteins
ribo.ind <- stringr::str_detect(fData(MENS)$gene_short_name,"^Rps") |
  stringr::str_detect(fData(MENS)$gene_short_name,"^Rpl")

ribo.ind <- stringr::str_detect(fData(MENS)$gene_short_name,"^mt-") 

#Select genes with expression in greater than 10 cells
ribo.ind.filt <- which(fData(MENS)$num_cells_expressed[ribo.ind] > 10)
MENS_sub <- MENS[ribo.ind.filt,]

ribo_tx_counts <- data.frame("cluster" = NA,"norm" = NA ,"tx_count" = NA)
for(cl in sort(unique(pData(MENS_sub)$clusters))){
  raw <- sum(exprs(MENS_sub[,pData(MENS_sub)$clusters == cl]))
  raw_avg <- raw/sum(pData(MENS_sub)$clusters == cl)

  #use the size normalization factor computed for all genes, do not recompute here
  size_norm <- sum(normalized_counts(MENS_sub[,pData(MENS_sub)$clusters == cl], norm_method = "size_only"))
  size_norm_avg <- size_norm/sum(pData(MENS_sub)$clusters == cl)

  df <- data.frame(cluster = cl, norm = c("raw","size_normed"), tx_count = c(raw_avg, size_norm_avg))
  ribo_tx_counts <- rbind(ribo_tx_counts, df)
}
ribo_tx_counts <- ribo_tx_counts[-c(1),]
ggplot(ribo_tx_counts, aes(cluster, tx_count)) + geom_col(aes(fill = norm), position = "dodge") + 
  ggtitle("Average ribosomal transcripts per cell")


MENS_sub <- estimate_size_factors(MENS_sub)
MENS_sub <- preprocess_cds(MENS_sub, num_dim = 5, verbose=TRUE)
MENS_sub <- reduce_dimension(MENS_sub,max_components=2,cores=4,verbose=TRUE)
MENS_sub <- align_cds(MENS_sub, num_dim = 5, alignment_group = "batch",alignment_k = 20,verbose=T)
MENS_sub <- reduce_dimension(MENS_sub,max_components=2,cores=4,verbose=TRUE)
MENS_sub <- cluster_cells(MENS_sub, k=20, cluster_method="leiden",num_iter=5, reduction_method="UMAP", partition_qval=0.05,verbose=TRUE,resolution=1e-04)
plot_cells(MENS_sub)
...
```{r, eval = F}
#Grab genes that are annotated to the cell cycle via GO
cycle.anno <- AnnotationDbi::select(org.Mm.eg.db, keytype='GOALL', keys='GO:0007049',columns='ENSEMBL')[,'ENSEMBL']

cycle.genes <- rownames(MENS)[fData(cds)$gene_id_trimmed %in% cycle.anno]
                     
MENS <- preprocess_cds(MENS, num_dim = 5, use_genes = cycle.genes, verbose=TRUE)
MENS <- reduce_dimension(MENS,max_components=2,cores=4,verbose=TRUE, preprocess_method = "PCA")

plot_cells(MENS, color_cells_by = "assigned_cluster")
plot_cells(MENS, color_cells_by = "cycle_G2M_score")
```
...
normed_exprs <- log10(as.matrix(exprs(MENS))+1)
...
pdf(paste0("./plots/NMF/MENS/MENS_",date,"_k_40_log_cellweights.pdf"), width = 10)
...
if(plot_over_time){
  ...
  myplots <- lapply(paste0("cellPattern",condition_patterns), 
                    ...
                    cds = MENS,
                    bin_by = "age")
  pdf(paste0("./plots/NMF/MENS/MENS_",date,"_patterns_over_age.pdf"), height= 20, width = 20)
...
#saveWidget(dt, file = "./results/NMF/MENS/MENS_10-31_pattern_gene_weights.html")

write.csv(cellWeights.df, paste0("./results/NMF/MENS/MENS_",date,"_pattern_cell_weights.csv"))

write.csv(geneWeights.df, paste0("./results/NMF/MENS/MENS_",date,"_pattern_gene_weights.csv"))
...
pdf(paste0("./plots/NMF/MENS/MENS_",date,"_pattern_feature_correlation.pdf"), height= 8, width = 8)
...
geneSetEnrichment(gene_weights = geneWeights.df,
                    n_genes = 1500, n_patterns = 40, file_prefix = paste0("MENS_",date,"_pattern_gene_weights"))

```

```{r}
saveRDS(MENS, file = "/home/jared/ENS/Timecourse_ENS/TC_MENS_11_5.rds")
sessionInfo()
```
