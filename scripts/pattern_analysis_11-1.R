...
if(do.existing.patterns){
  date <- ""
  gene_weights <- read.csv(paste0(existing_pattern_path, "pattern_gene_weights.csv"), row.names = 1)
  cell_weights <- read.csv(paste0(existing_pattern_path, "pattern_cell_weights.csv"), row.names = 1)
} else{
...
pdf("/home/jared/ENS/6mo_LMMP/plots/NMF/lmmp/NMF_lmmp_6mo_11-1_cellweights.pdf", width = 10)
...
weighted_emb <- lapply(1:npattern,
                       ...
                       red_method = "UMAP",
                       do.clip = c(0.02,.98))
png("/home/jared/ENS/6mo_LMMP/plots/NMF/lmmp/lmmp_6mo_11-1_NMF_Patterns.png", height= 2000, width = 2000)
...
if(plot_over_time){
  ...
  myplots <- lapply(paste0("cellPattern",condition_patterns), 
                    ...
                    cds = lmmp_6mo,
                    bin_by = "sample")
  pdf("/home/jared/ENS/6mo_LMMP/plots/NMF/lmmp/lmmp_6mo_11-1_patterns_over_sample.pdf", height= 20, width = 20)
...
saveWidget(dt, file = "/home/jared/ENS/6mo_LMMP/results/NMF/lmmp/lmmp_6mo_11-1_pattern_gene_weights.html")

write.csv(cellWeights.df, "/home/jared/ENS/6mo_LMMP/results/NMF/lmmp/lmmp_6mo_11-1_pattern_cell_weights.csv")

write.csv(geneWeights.df, "/home/jared/ENS/6mo_LMMP/results/NMF/lmmp/lmmp_6mo_11-1_pattern_gene_weights.csv")
...
geneSetEnrichment(gene_weights = geneWeights.df,
                  n_genes = 1000, n_patterns = 50, file_prefix = "/home/jared/ENS/6mo_LMMP/results/GSEA/lmmp/lmmp_6mo_11-1_")
...
pdf("/home/jared/ENS/6mo_LMMP/plots/NMF/lmmp/LMMP_11-1_pattern_categorical_feature_correlation.pdf",
    width = 18, height = 10)
heatmaps
dev.off()
